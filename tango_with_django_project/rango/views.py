...
def index(request):
	...
	# Suppose I have to order the pages according to the view preference
	#pages_list = Page.objects.order_by('-views')[:5]
...
def show_category(request, category_name_slug):
	...
	# Go render the request and return it to the client
...
def register(request):
	...
	if request.method == 'POST':
		...
		if user_form.is_valid() and profile_form.is_valid():
			...
			#Now hashed, we can update the user object.
			user.set_password(user.password)
			user.save()

			#Now sort out the UserProfile instance.
			#Since we need to set the user attribute ourselves.
			#we set commit=False. This delays saving the models
